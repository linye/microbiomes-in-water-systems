    ...
    with open(fasta_file, 'r') as f:
        for line in f:
            ...
            if line.startswith('>'):
                ...
                # Extract sequence ID (part before first space)
...
def count_tes_per_plasmid(blast_result_file):
    """Count the number of TEs (transposable elements) matched per plasmid"""
    plasmid_te_count = defaultdict(int)
    ...
    with open(blast_result_file, 'r') as f:
        for line in f:
            ...
            if orf_name not in processed_orfs:
                plasmid_te_count[plasmid_id] += 1
                processed_orfs.add(orf_name)

    return plasmid_te_count
...
def generate_summary_table(original_fasta, orf_fasta, blast_result, output_file):
    ...
    # Get TE counts per plasmid
    te_counts = count_tes_per_plasmid(blast_result)
    print(f"Counted TEs for {len(te_counts)} plasmids")

    # Generate summary table
    with open(output_file, 'w') as out:
        # Write header
        out.write("Plasmid_ID\tLength\tORF_Count\tTE_Count\n")

        # Write data
        for plasmid_id in plasmid_lengths:
            length = plasmid_lengths.get(plasmid_id, 0)
            orf_count = orf_counts.get(plasmid_id, 0)
            te_count = te_counts.get(plasmid_id, 0)

            out.write(f"{plasmid_id}\t{length}\t{orf_count}\t{te_count}\n")
...
def main():
    ...
    target_blast_result = "plasmid_30_new_te_cov70.txt"
    target_output = "plasmid_30_new_te.txt"
    ...
    random_blast_result = "plasmid_random_1000_te_cov70.txt"
    random_output = "plasmid_random_1000_te.txt"
