...
def count_vfs_per_plasmid(blast_result_file):
    """Count the number of VFs (virulence factors) matched per plasmid"""
    plasmid_vf_count = defaultdict(int)
    ...
    with open(blast_result_file, 'r') as f:
        for line in f:
            ...
            if orf_name not in processed_orfs:
                plasmid_vf_count[plasmid_id] += 1
                processed_orfs.add(orf_name)

    return plasmid_vf_count
...
def generate_summary_table(original_fasta, orf_fasta, blast_result, output_file):
    ...
    # Get VF counts per plasmid
    vf_counts = count_vfs_per_plasmid(blast_result)
    print(f"Counted VFs for {len(vf_counts)} plasmids")

    # Generate summary table
    with open(output_file, 'w') as out:
        # Write header
        out.write("Plasmid_ID\tLength\tORF_Count\tVF_Count\n")

        # Write data
        for plasmid_id in plasmid_lengths:
            length = plasmid_lengths.get(plasmid_id, 0)
            orf_count = orf_counts.get(plasmid_id, 0)
            vf_count = vf_counts.get(plasmid_id, 0)

            out.write(f"{plasmid_id}\t{length}\t{orf_count}\t{vf_count}\n")
...
def main():
    ...
    target_blast_result = "plasmid_30_new_vf_cov70.txt"
    target_output = "plasmid_30_new_vf.txt"

    # Random plasmid data file paths
    random_plasmid_fasta = "../random/plasmid_random_1000.fasta"
    random_orf_fasta = "../plasmid_random_1000_nucleotide_seq.fasta"
    random_blast_result = "plasmid_random_1000_vf_cov70.txt"
    random_output = "plasmid_random_1000_vf.txt"
