...
def load_data():
    """Load VF data files"""
    # Read the two VF statistics files
    shared_df = pd.read_csv('plasmid_30_new_vf.txt', sep='\t')
    random_df = pd.read_csv('plasmid_random_1000_vf.txt', sep='\t')
    ...
    # Calculate VF/ORF ratio (avoid division by zero)
    shared_df['VF_ORF_ratio'] = shared_df['VF_Count'] / shared_df['ORF_Count'].replace(0, np.nan)
    random_df['VF_ORF_ratio'] = random_df['VF_Count'] / random_df['ORF_Count'].replace(0, np.nan)

    return shared_df, random_df


def plot_vf_orf_ratio_boxplot(shared_df, random_df):
    """Draw boxplot of VF/ORF ratio (log scale)"""
    # Prepare data
    ...
    for _, row in small_plasmids.iterrows():
        if not pd.isna(row['VF_ORF_ratio']) and row['VF_ORF_ratio'] > 0:
            plot_data.append({'type': 'small-plasmid', 'ratio': row['VF_ORF_ratio']})

    for _, row in large_plasmids.iterrows():
        if not pd.isna(row['VF_ORF_ratio']) and row['VF_ORF_ratio'] > 0:
            plot_data.append({'type': 'large-plasmid', 'ratio': row['VF_ORF_ratio']})

    for _, row in shared_df.iterrows():
        if not pd.isna(row['VF_ORF_ratio']) and row['VF_ORF_ratio'] > 0:
            plot_data.append({'type': 'shared-plasmid', 'ratio': row['VF_ORF_ratio']})

    plot_df = pd.DataFrame(plot_data)

    if len(plot_df) == 0:
        print("Warning: No valid VF/ORF ratio data to plot")
        return
    ...
    plt.ylabel('VF/ORF Ratio (log scale)')
    plt.title('VF/ORF Ratio Comparison by Plasmid Type (Log Scale)')
    plt.xticks(rotation=45)
    plt.tight_layout()

    plt.savefig('vf_orf_ratio_boxplot_log.png', dpi=300, bbox_inches='tight')
    plt.savefig('vf_orf_ratio_boxplot_log.pdf', bbox_inches='tight')
    plt.close()

    # Print statistics
    print("\nVF/ORF ratio statistics:")
...
def additional_analysis(shared_df, random_df):
    ...
    # 1. VF prevalence analysis
    print("\n=== VF Prevalence Analysis ===")
    for name, df, desc in [('shared', shared_df, 'Shared plasmids'),
        ...
        vf_positive = len(df[df['VF_Count'] > 0])
        total = len(df)
        if total > 0:
            print(f"{desc}: {vf_positive}/{total} ({vf_positive / total * 100:.1f}%) carry VFs")

    # 2. Correlation between length and VF count
    print("\n=== Correlation between Length and VF Count ===")
    for name, df in [('Shared', shared_df), ('Random', random_df)]:
        if len(df) > 1:
            vf_positive_df = df[df['VF_Count'] > 0]
            if len(vf_positive_df) > 1:
                corr = vf_positive_df['Length'].corr(vf_positive_df['VF_Count'])
                print(f"{name} plasmids - correlation (VF-positive only): {corr:.3f}")
            corr_all = df['Length'].corr(df['VF_Count'])
...
def statistical_tests(shared_df, random_df):
    ...
    # 2. VF/ORF ratio difference test
    groups = {
        'small-plasmid': small_plasmids['VF_ORF_ratio'].dropna(),
        'large-plasmid': large_plasmids['VF_ORF_ratio'].dropna(),
        'shared-plasmid': shared_df['VF_ORF_ratio'].dropna()
    }
    ...
    if len(valid_groups) >= 2:
        ...
        print(f"VF/ORF ratio difference among plasmid types: p-value = {p_value:.4f}")
...
def generate_summary_report(shared_df, random_df):
    ...
    print("\n" + "=" * 60)
    print("                    VF Data Analysis Summary Report")
    print("=" * 60)
    ...
    print(f"\n3. VF carriage:")
    shared_vf_positive = len(shared_df[shared_df['VF_Count'] > 0])
    random_vf_positive = len(random_df[random_df['VF_Count'] > 0])
    print(
        f"   Shared plasmids VF-positive: {shared_vf_positive}/{len(shared_df)} ({shared_vf_positive / len(shared_df) * 100:.1f}%)")
    print(
        f"   Random plasmids VF-positive: {random_vf_positive}/{len(random_df)} ({random_vf_positive / len(random_df) * 100:.1f}%)")

    print(f"\n4. VF/ORF ratio summary:")
    small_plasmids = random_df[random_df['size_category'] == 'small-plasmid']
    large_plasmids = random_df[random_df['size_category'] == 'large-plasmid']
...
def main():
    ...
    if not os.path.exists('plasmid_30_new_vf.txt') or not os.path.exists('plasmid_random_1000_vf.txt'):
        print("Error: Please ensure the VF statistics files exist.")
        print("Required files:")
        print("1. plasmid/random/plasmid_30_new_vf.txt")
        print("2. plasmid_random_1000_vf.txt")
        return

    print("Starting VF data analysis...")

    # Load data
    shared_df, random_df = load_data()

    # Generate boxplot (log scale)
    plot_vf_orf_ratio_boxplot(shared_df, random_df)
    ...
    print("\nVF analysis complete! Generated plots:")
    print("- vf_orf_ratio_boxplot_log.png/pdf: Boxplot of VF/ORF ratio (log scale)")
